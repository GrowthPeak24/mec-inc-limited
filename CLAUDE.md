...
- **Only six client components ship sitewide:** `MobileNav`, `Reveal`, `QuoteBuilder` (+ its 4 steps), `ContactForm`, `FeedbackForm`, `CopyReference`. Everything else must remain a Server Component. Adding a 7th `'use client'` is a design decision, not a routine change. `FeedbackForm` was the deliberate breach of the original 4-ceiling — the star-rating picker needs interactive hover/keyboard preview, and reusing the ContactForm progressive-enhancement pattern (native form + Server Action + honeypot + dwell) keeps every write-surface on the same mental model. `CopyReference` (`src/components/quote/CopyReference.tsx`) is the 6th — the clipboard button on `/quote/thank-you`. It is a leaf: the confirmation page stays a Server Component, and the reference always renders as plain selectable text beside the button, so an absent/blocked `navigator.clipboard` degrades to manual copy instead of trapping the user.
